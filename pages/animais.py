...
def main():
    ...
    with col3:
        st.metric(
            "Rendimento por venda", round(animais["Rendimento de venda"].mean(), 2)
        )
        st.metric(
            "Rendimento médio de produção",
            round(animais["Rendimento de produção"].mean(), 2),
        )

    st.dataframe(
        animais, hide_index=True, on_select="rerun", selection_mode="multi-row"
    )

    animais = animais.sort_values(by="Rendimento de produção", ascending=False).dropna(
        subset="Custo"
    )
    ...
    st.subheader("Gráfico de rendimento por produção por animal")
    fig, ax = plt.subplots()
    # Agrupar por 'Produz_item' e calcular a média de 'Rendimento de produção'
    animais_grouped = (
        animais.groupby("Produz_item")["Rendimento de produção"].mean().reset_index()
    )
    # Ordenar por 'Rendimento de produção'
    animais_grouped = animais_grouped.sort_values(
        by="Rendimento de produção", ascending=False
    )
