...
def pegar_condicao(x):
    ...
    if len(x) > 1:
        for i in range(
            len(x) - 1, -1, -1
        ):  # retornar o item antes do último número inteiro
            try:
                int(x[i])
                cond = x[i - 1]
                break
...
def xp():
    ...
    mineracao["Experiências"] = mineracao.Experiências.apply(
        lambda lista: transforma_lista(lista)
    )
    mineracao["XP"] = mineracao.Experiências.apply(lambda x: pegar_primeiro_inteiro(x))
    mineracao["estacao_condicao"] = mineracao.Experiências.apply(
        lambda x: pegar_condicao(x)
    )
    mineracao["XP_condicional"] = mineracao.Experiências.apply(
        lambda x: pegar_inteiro_cond(x)
    )
    ...
    mineracao = mineracao[
        ["Profissao", "item", "XP", "estacao_condicao", "XP_condicional"]
    ]
    ...
    coleta.loc[0:3, "item"] = coleta.loc[0:3, "item"].apply(
        lambda linha: linha.split("por")[1]
    )
    ...
    df_pesca_melt["item"] = (
        "Nível "
        + df_pesca_melt["Nível"].astype(str)
        + " pescando "
        + df_pesca_melt["peixe"]
    )
...
def precos_carrinho_viagem(
    elemento: str, separador: str = "–", minimo: bool = False, maximo: bool = False
):
...
def separar_dias(valor):
  ...
  """
    if "-" in str(valor):  # por intervalo de datas com '-'
        partes = valor.split("-")
        lista = list(
            range(int(partes[0]), int(partes[1]) + 1)
...
def separar_quantidades_e_explodir(df, coluna, divisor: str = ")"):
    ...
    df[coluna] = df[coluna].apply(
        lambda linha: tenta_separar_string(linha=linha, divisor=divisor)
    )  # separa itens
    df = df.explode(coluna).dropna(subset=coluna)
    df[coluna] = df[coluna].apply(
        lambda linha: linha.split("(") if "(" in linha else [linha, "0"]
    )  # separa qunatidades
...
def divide_valores_por_qualidade(
    df, coluna_nome, coluna_valor, coluna_energia_saude=None
):
    """
    ...
    for i in range(0, len(df)):
        if i % 4 == 0:
            df.loc[i, coluna_valor] = tenta_dividir(
                texto_a_dividir=df.loc[i, coluna_valor], divisor="ouros", coluna=0
            )
            if coluna_energia_saude:
                df.loc[i, "Energia"] = tenta_dividir(
                    texto_a_dividir=df.loc[i, coluna_energia_saude], coluna=0
                )
                df.loc[i, "Saude"] = tenta_dividir(
                    texto_a_dividir=df.loc[i, coluna_energia_saude], coluna=1
                )
        if i % 4 == 1:
            df.loc[i, coluna_nome] = df.loc[i, coluna_nome] + "_prata"
            df.loc[i, coluna_valor] = tenta_dividir(
                texto_a_dividir=df.loc[i, coluna_valor], divisor="ouros", coluna=1
            )
            if coluna_energia_saude:
                df.loc[i, "Energia"] = tenta_dividir(
                    texto_a_dividir=df.loc[i, coluna_energia_saude], coluna=2
                )
                df.loc[i, "Saude"] = tenta_dividir(
                    texto_a_dividir=df.loc[i, coluna_energia_saude], coluna=3
                )
        if i % 4 == 2:
            df.loc[i, coluna_nome] = df.loc[i, coluna_nome] + "_ouro"
            df.loc[i, coluna_valor] = tenta_dividir(
                texto_a_dividir=df.loc[i, coluna_valor], divisor="ouros", coluna=2
            )
            if coluna_energia_saude:
                df.loc[i, "Energia"] = tenta_dividir(
                    texto_a_dividir=df.loc[i, coluna_energia_saude], coluna=4
                )
                df.loc[i, "Saude"] = tenta_dividir(
                    texto_a_dividir=df.loc[i, coluna_energia_saude], coluna=5
                )
        if i % 4 == 3:
            df.loc[i, coluna_nome] = df.loc[i, coluna_nome] + "_iridio"
            df.loc[i, coluna_valor] = tenta_dividir(
                texto_a_dividir=df.loc[i, coluna_valor], divisor="ouros", coluna=3
            )
            if coluna_energia_saude:
                df.loc[i, "Energia"] = tenta_dividir(
                    texto_a_dividir=df.loc[i, coluna_energia_saude], coluna=6
                )
                df.loc[i, "Saude"] = tenta_dividir(
                    texto_a_dividir=df.loc[i, coluna_energia_saude], coluna=7
                )
    return df
...
def concat_dataframes():
    ...
    df_animais["Produz"] = df_animais["Produz"].apply(
        lambda linha: linha.replace("Avestruz", "avestruz").replace(
            "Dourado", "dourado"
        )
    )
    ...
    df_animais["Rendimento de venda"] = (
        df_animais["Venda_5_coracoes"] / df_animais["Custo"]
    )
    df_animais["Rendimento de produção"] = (
        df_animais["Custo"] / df_animais["Produz_valor"]
    )
    ...
    for arma in lista_armas:
        ...
        df_temp["Tipo"] = arma
        if arma == "municoes":
            df_temp["Descrição"] = df_temp["Multiplicador de Munição"].apply(
                lambda x: "Base de dano: " + str(x)
            )
            df_temp["Item"] = df_temp["Item"].apply(
                lambda linha: str(
                    linha.replace("Todas as  ", "")
                    .replace("  • Todos os ", ", ")
                    .replace('"', "")
                )
            )
    ...
    # atributos luta
    df_atributos_luta = pd.read_csv(
        "docs_bronze/armas_luta_atributos.csv", encoding="utf-8"
    )
    ...
    df_arvores = pd.concat(
        [df.reset_index(drop=True) for df in dfs_to_concat] * 4, ignore_index=True
    ).sort_values(by="Fruta")
    df_arvores = df_arvores.reset_index(drop=True)
    df_arvores = divide_valores_por_qualidade(
        df=df_arvores,
        coluna_nome="Fruta",
        coluna_valor="Preço de venda",
        coluna_energia_saude="Efeito de Cura",
    )
    df_arvores = df_arvores.rename(
        columns={
            "Preço da Muda": "preco_muda_pierre",
            "Preço da Muda.1": "preco_muda_carrinho_viagem",
            "Preço de venda": "Preco_venda",
        }
    )
    df_arvores.preco_muda_pierre = limpar_preco(df_arvores.preco_muda_pierre)
    df_arvores.preco_muda_carrinho_viagem = limpar_preco(
        df_arvores.preco_muda_carrinho_viagem
    )
    df_arvores["preco_minimo_muda_carrinho_viagem"] = (
        df_arvores.preco_muda_carrinho_viagem.apply(precos_carrinho_viagem, minimo=True)
    )
    df_arvores["preco_maximo_muda_carrinho_viagem"] = (
        df_arvores.preco_muda_carrinho_viagem.apply(precos_carrinho_viagem, maximo=True)
    )
    df_arvores["Preco_venda"] = df_arvores["Preco_venda"].astype(float)
    for linha in df_arvores.index:
        try:
            df_arvores.loc[linha, "Rentabilidade"] = round(
                df_arvores.loc[linha, "Preco_venda"]
                / df_arvores.loc[linha, "preco_muda_pierre"]
                * 1000,
                2,
            )
        except ZeroDivisionError:
            df_arvores.loc[linha, "Rentabilidade"] = 0
    df_arvores = df_arvores[
        [
            "Fruta",
            "Muda",
            "Preco_venda",
    ...
    # vestuario
    lista_vestuario = ["aneis", "calcados", "chapeus"]
    dfs_to_concat = []
    ...
    df_calendario["Mes"] = df_calendario["Mes"].apply(
        lambda linha: linha.replace("0.0.", "0.")
        .replace("1.0.", "1.")
        .replace("2.0.", "2.")
    )
    ...
    df_carteira = pd.concat(
        [carteira_especiais, carteira_livros, carteira_poderes], ignore_index=True
    ...
    caverna_morcego = caverna_morcego.rename(
        columns={"Renda": "Lucro", "Chance": "Chance_%"}
    )
    df_caverna = (
        pd.concat([caverna_cogumelo, caverna_morcego] * 4, ignore_index=True)
        .reset_index(drop=True)
        .sort_values(by="Nome")
    )
    df_caverna = df_caverna.dropna(
        subset=["Descrição"], ignore_index=True
    )  # apaga linhas onde Descrição está vazio
    df_caverna = divide_valores_por_qualidade(
        df=df_caverna,
        coluna_nome="Nome",
        coluna_valor="Lucro",
        coluna_energia_saude="Recupera",
    )
    df_caverna["Energia"] = df_caverna["Energia"].apply(
        lambda linha: linha.replace("−", "-") if isinstance(linha, str) else linha
    )
    df_caverna["nome_original"] = df_caverna["Nome"].apply(
        lambda linha: linha.split("_")[0]
    )
    df_caverna["Chance_%"] = df_caverna["Chance_%"].apply(
        lambda linha: linha.replace("%", "") if isinstance(linha, str) else linha
    )
    df_caverna = df_caverna[
        [
            "nome_original",
            "Nome",
            "Lucro",
    ...
    df_coleta = (
        df_coleta.drop(index=[49, 50, 51, 121, 122, 123, 157, 158, 159])
        .dropna(subset=["Descrição"], ignore_index=True)
        .reset_index(drop=True)
    )
    df_coleta["Lucro"] = limpar_preco(df_coleta["Lucro"])
    df_coleta["Energia"] = df_coleta["Energia"].apply(
        lambda linha: linha.replace("−", "-") if isinstance(linha, str) else linha
    )
    df_coleta["Energia"] = df_coleta["Energia"].apply(
        lambda linha: 0 if (linha == "Não comestível" or linha == "Não") else linha
    )
    df_coleta["Saude"] = df_coleta["Saude"].apply(
        lambda linha: (
            0 if (linha == "Não comestível" or linha == "comestível") else linha
        )
    )
    ...
    lista_conjunto = [
        ...
        "pesca_noturna",
        "pesquisa_campo",
        "plantacoes_primavera",
        "plantacoes_verao",
        "plantacoes_outono",
        "plantacoes_qualidade",
        "recursos_exoticos",
        "tinta",
    ]
    dfs_to_concat = []
    for conjunto in lista_conjunto:
        ...
        df_temp.columns = [
            "Conjunto",
            "Requisitos",
            "Descrição_requisitos",
            "Recompensa",
        ]
        dfs_to_concat.append(df_temp)
    df_conjuntos = pd.concat(dfs_to_concat, ignore_index=True)
    df_conjuntos["Requisitos"] = df_conjuntos.apply(
        lambda row: (
            row["Requisitos"]
            if not pd.isna(row["Requisitos"])
            else row["Descrição_requisitos"]
        ),
        axis=1,
    )
    ...
    for culinaria in lista_culinaria:
        ...
        df_temp = df_temp.rename(
            columns={
                "Necessário para:": "Necessário para",
    ...
    df_culinaria["Energia"] = df_culinaria["Energia / Saúde"].apply(
        lambda row: tenta_dividir(texto_a_dividir=row, divisor="  ", coluna=0)
    )
    df_culinaria["Saúde"] = df_culinaria["Energia / Saúde"].apply(
        lambda row: tenta_dividir(texto_a_dividir=row, divisor="  ", coluna=1)
    )
    df_culinaria["Preço de Venda Condicional"] = df_culinaria["Preço de Venda"].apply(
        lambda linha: (
            linha.split("/")[1].strip().split("(")[0] if "/" in str(linha) else None
        )
    )
    df_culinaria["Preço de Venda Condicional"] = limpar_preco(
        df_culinaria["Preço de Venda Condicional"]
    )
    df_culinaria["Condição de preço condicional"] = df_culinaria[
        "Preço de Venda"
    ].apply(
        lambda linha: (
            linha.split("/")[1].strip().split("(")[1].replace(")", "")
            if "(" in str(linha)
            else None
        )
    )
    df_culinaria["Preço de Venda"] = df_culinaria["Preço de Venda"].apply(
        lambda linha: linha.split("/")[0].strip() if "/" in str(linha) else linha
    )
    df_culinaria["Preço de Venda"] = limpar_preco(df_culinaria["Preço de Venda"])
    linhas_a_retirar = (df_culinaria["Fonte"].isna()) & (
        df_culinaria["Necessário para"].isna()
    )
    df_culinaria = df_culinaria[~linhas_a_retirar]
    df_culinaria = df_culinaria.drop(
        columns=["Energia / Saúde", "Quantidade necessária"]
    )

    df_culinaria.to_csv("docs_silver/culinaria.csv", encoding="utf-8")

    # custo_solo_concha
    solo_concha = pd.read_csv(
        f"docs_bronze\custo_solo_foliar_concha.csv", header=2, index_col="1"
    )
    ...
    # custo_solo_coral
    solo_coral = pd.read_csv(
        f"docs_bronze\custo_solo_foliar_concha.csv", header=11, index_col="10"
    )
    solo_coral["Tipo"] = "Coral"
    solo_coral = solo_coral.rename(columns={"Qualidade do Coral": "Qualidade"})
    # concatena
    df_solo = pd.concat([solo_concha, solo_coral], ignore_index=True).reset_index(
        drop=True
    )
    df_solo.columns = [
        "Profissão",
    ...
    df_solo = df_solo[
        ["Tipo", "Qualidade", "Profissão", "Artesanato", "Pierre", "Oásis"]
    ]
    ...
    df_casa = df_casa[
        ["Tipo", "Nome", "Custo", "Animais", "Descrição", "Tamanho", "Detalhes"]
    ]
    # df_casa = separar_quantidades_e_explodir(df= df_casa, coluna='Custo', divisor=')')
    # onde Tipo = estagios, adicionar 'Estágio ' ao nome
    df_casa.loc[df_casa.Tipo == "estagios", "Nome"] = "Estágio " + df_casa.loc[
        df_casa.Tipo == "estagios", "Nome"
    ].astype(str)
    ...
    df_ferramentas = df_ferramentas[
        ["Tipo", "Nome", "Efeito", "Preço", "Materiais", "Localização", "Requisitos"]
    ]
    ...
    df_iscas = df_iscas.rename(
        columns={"Compra": "Custo (Ouros)", "Produção": "Custo (produtos)"}
    )
    ...
    for lavoura in lista_lavouras:
        ...
        dfs_to_concat.append(df_temp)
    df_lavouras = (
        pd.concat(dfs_to_concat * 4, ignore_index=True)
        .sort_values(by="Semente")
        .reset_index(drop=True)
    )
    df_lavouras.columns = [
        "Apagar",
        "Crescimento estágio 1 (dias)",
        "Crescimento estágio 2 (dias)",
        "Crescimento estágio 3 (dias)",
        "Tempo colheita (dias)",
        ...
        "Crescimento estágio 4 (dias)",
        "Crescimento estágio 5 (dias)",
    ]

    # consultar em qual coluna está o texto Total: e alocar na nova coluna 'Crescimento Total (dias)'
    df_lavouras["Crescimento Total (dias)"] = df_lavouras.apply(
        extrair_valor_total, axis=1
    )
    df_lavouras = divide_valores_por_qualidade(
        df=df_lavouras,
        coluna_nome="Semente",
        coluna_valor="Vende_por",
        coluna_energia_saude="Restaura",
    )
    df_lavouras = df_lavouras[
        [
            "Semente",
            "Estação",
            "Vende_por",
    ...
    df_lavouras["Energia"] = df_lavouras["Energia"].apply(
        lambda linha: linha.replace("−", "-") if isinstance(linha, str) else linha
    )
    df_lavouras["Energia"] = df_lavouras["Energia"].apply(
        lambda linha: 0 if (linha == "Não comestível" or linha == "Não") else linha
    )
    df_lavouras["Saude"] = df_lavouras["Saude"].apply(
        lambda linha: (
            0 if (linha == "Não comestível" or linha == "comestível") else linha
        )
    )
    # limpar dias em estágios de crescimento

    df_lavouras.to_csv("docs_silver/lavouras.csv", encoding="utf-8")

    # lista_presente
    df_presentes = pd.read_csv(
        "docs_bronze\lista_presentes.csv", encoding="utf-8", index_col="Unnamed: 0"
    )
    ...
    for nome in df_mercadorias["Nome"].unique():
        if nome == "Óleo":
            ...
            df_mercadorias.loc[df_mercadorias["Nome"] == nome] = df_mercadorias.loc[
                df_mercadorias["Nome"] == nome
            ].bfill()
            df_mercadorias.loc[df_mercadorias["Nome"] == nome] = df_mercadorias.loc[
                df_mercadorias["Nome"] == nome
            ].drop_duplicates(subset="Nome", keep="first")
    df_mercadorias = df_mercadorias.dropna(
        subset=["Descrição"], ignore_index=True
    ).reset_index(drop=True)
    df_mercadorias.to_csv("docs_silver/mercadorias.csv", encoding="utf-8")

    # minerais
    lista_minerais = ["coleta", "gemas", "geodos", "origem_geodos"]
    dfs_to_concat = []
    ...
    dfs_to_concat.append(
        pd.read_csv("docs_bronze/nos_minerio.csv").rename(
            columns={"Local": "Localização"}
        )
    )
    ...
    for mobilia in lista_mobilias:
        if mobilia == "tapetes":
            ...
            df_temp.columns = [
                ...
                "Nome",
                "carpintaria(ouros)",
                "carrinho_viagem(ouros)",
                "pierre(ouros)",
                "oasis",
                "Tamanho",
            ]
            df_temp = df_temp.melt(
                id_vars=["Nome", "Tamanho"],
                value_vars=[
                    "carpintaria(ouros)",
                    "carrinho_viagem(ouros)",
                    "pierre(ouros)",
        ...
        if mobilia == "aquarios":
            ...
            df_temp = df_temp.melt(
                id_vars=["Nome"],
                value_vars=["peixaria(ouros)", "catalogo(ouros)", "qi(diamantes)"],
                var_name="Loja",
                value_name="Preço",
            )
        if mobilia in [
        ...
        if mobilia == "mesas_longas":
            ...
            df_temp = df_temp.melt(
                id_vars=["Nome"],
                value_vars=["carpintaria(ouros)", "festivais(ouros)"],
                var_name="Loja",
                value_name="Preço",
            )
        if mobilia == "lareiras":
            ...
            df_temp = df_temp.melt(
                id_vars=["Nome"],
                value_vars=["carpintaria(ouros)", "outros"],
                var_name="Loja",
                value_name="Preço",
            )
        if mobilia == "lampadas":
            ...
            df_temp = df_temp.melt(
                id_vars=["Nome"],
                value_vars=["carpintaria(ouros)", "carrinho_viagem(ouros)", "oasis"],
                var_name="Loja",
                value_name="Preço",
            )
        if mobilia == "janelas":
            ...
            df_temp = df_temp.melt(
                id_vars=["Nome"],
                value_vars=["carpintaria(ouros)", "pierre(ouros)"],
                var_name="Loja",
                value_name="Preço",
            )
        if mobilia == "tvs":
            df_temp.columns = [
                ...
                "Nome",
                "carpintaria(ouros)",
                "comerciante_ilhas(inhame_coco)",
            ]
            df_temp = df_temp.melt(
                id_vars=["Nome"],
                value_vars=["carpintaria(ouros)", "comerciante_ilhas(inhame_coco)"],
                var_name="Loja",
                value_name="Preço",
            )
        if mobilia in ["plantas_decorativas_chao1", "plantas_decorativas_chao2"]:
            ...
            df_temp = df_temp.melt(
                id_vars=["Nome"],
                value_vars=["carpintaria(ouros)", "carrinho_viagem(ouros)"],
                var_name="Loja",
                value_name="Preço",
            )
        if mobilia == "plantas_decorativas_sazonais":
            df_temp = df_temp[["Também vendido por", "Preço"]].rename(
                columns={"Também vendido por": "Loja"}
            )
    ...
    df_mobilias[["valor", "moeda"]] = df_mobilias["Preço"].apply(
        lambda x: pd.Series(extrair_valor_moeda(x))
    )
    ...
    df_mobilias["Tipo"] = df_mobilias["Tipo"].apply(
        lambda row: row.replace("1", "").replace("2", "").replace("3", "")
    )
    df_mobilias["Nome"] = df_mobilias["Nome"].apply(
    ...
    df_peixes = divide_valores_por_qualidade(
        df=df_peixes,  # falta explodir Marinheiro e Não-marinheiro junto ver multiplicação
        coluna_nome="Nome",
    ...
    pescas_zona.columns = [
        ...
        "Zona de Pesca",
        "Nível de Pesca",
        "Tamanho mínimo de peixe",
        "Tamanho máximo de peixe",
        "% Qualidade Normal",
        "% Qualidade Prata",
        "% Qualidade Ouro",
    ...
    for taxa in lista_taxas_cultivo:
        ...
        dfs_to_concat.append(df_temp)
        i += 1
    df_taxas_cultivo = pd.concat(dfs_to_concat, ignore_index=True).reset_index(
        drop=True
    )
    df_taxas_cultivo = df_taxas_cultivo.drop(columns=["Unnamed: 0"])
    # ordenar por nível de cultivo e secundariamente por tipo
    df_taxas_cultivo = (
        df_taxas_cultivo.sort_values(by=["Tipo"])
        .sort_values(by=["Nível de Cultivo"])
        .reset_index(drop=True)
    )
    df_taxas_cultivo = df_taxas_cultivo[
        [
            "Nível de Cultivo",
            "Tipo",
            "Preço médio",
...
def todas_funcoes_enriquecimento():
    """Carrega todas funções na sequência"""
    profissoes()
    xp()
    concat_dataframes()
