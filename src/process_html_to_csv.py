...
def df_cultivo(html):
    ...
    df_solos = solo_normal.merge(solo_fertilizante_basico, how="outer").merge(
        solo_fertilizante_qualidade, how="outer"
    )
    ...
    df_solos = df_solos.rename(
        columns={
            "Nível Cultivo": "nivel_habilidade_cultivo",
    ...
    df_solos_regular = df_solos[
        ["Melhoramento", "nivel_habilidade_cultivo", "qualidade_regular"]
    ]
    df_solos_regular["qualidade_produto"] = "1.regular"
    df_solos_regular = df_solos_regular.rename(
        columns={"qualidade_regular": "percentual"}
    )

    df_solos_prata = df_solos[
        ["Melhoramento", "nivel_habilidade_cultivo", "qualidade_prata"]
    ]
    df_solos_prata["qualidade_produto"] = "2.prata"
    df_solos_prata = df_solos_prata.rename(columns={"qualidade_prata": "percentual"})

    df_solos_ouro = df_solos[
        ["Melhoramento", "nivel_habilidade_cultivo", "qualidade_ouro"]
    ]
    ...
    df_solos = (
        df_solos.sort_values(by=["qualidade_produto"])
        .sort_values(by=["nivel_habilidade_cultivo"])
        .reset_index(drop=True)
    )

    # corrigir tipos de dados nas colunas
    df_solos["percentual"] = (
        df_solos["percentual"].replace("%", "", regex=True).astype(int)
    )
    df_solos["nivel_habilidade_cultivo"] = df_solos["nivel_habilidade_cultivo"].astype(
        str
    )

    # reordenar colunas
    df_solos = df_solos[
        ["nivel_habilidade_cultivo", "Melhoramento", "qualidade_produto", "percentual"]
    ]
    ...
    xp_chirivia["XP"] = round(
        xp_chirivia["Experiência"] / xp_chirivia["Chirivias Colhidas no Total"], 0
    )
    xp_chirivia["Cultivo"] = xp_chirivia["Chirivias Colhidas no Total"].apply(
        lambda linha: f"Cada 1 das {linha} Chirivias coletadas"
    )
...
def df_mineracao(html):
    ...
    for tabela in sopa.find_all("table"):
        lista.append(
            pd.read_html(StringIO(str(tabela)))
        )  # , parser='lxml')) #só funciona se ativar .venv
    ...
    profissao_mineracao = pd.concat(
        [profissao_mineracao, profissao_mineracao_cont], axis=1
    )
...
def df_coleta(html):
    ...
    seiva_coleta.Efeito = seiva_coleta.Efeito.str.replace(
        "−", "-"
    ...
    ilha_gengibre_coleta = ilha_gengibre_coleta[
        ~ilha_gengibre_coleta["Descrição"].isna()
    ]
    ...
    xp_coleta["item"] = xp_coleta.item.apply(
        lambda x: x.replace("\n", "")
    )  # retira quebras de linha
    xp_coleta["item"] = xp_coleta.item.str.split().agg(
        " ".join
    )  # retira espaços múltiplos entre as palavras
...
def df_mercadorias_artesanais(html):
    ...
    produto_maquina_maionese.to_csv(
        "docs_bronze/mercadoria_produto_maquina_maionese.csv"
    )
...
def df_primavera(html):
    ...
    calendario_primavera_festivais.to_csv(
        "docs_bronze/calendario_primavera_festivais.csv"
    )
    calendario_primavera_aniversario = lista[2][0]
    calendario_primavera_aniversario.to_csv(
        "docs_bronze/calendario_primavera_aniversario.csv"
    )
    calendario_primavera_colheita_unica = lista[4][0]
    calendario_primavera_colheita_unica.to_csv(
        "docs_bronze/calendario_primavera_colheita_unica.csv"
    )
    calendario_primavera_colheita_multipla = lista[5][0]
    calendario_primavera_colheita_multipla.to_csv(
        "docs_bronze/calendario_primavera_colheita_multipla.csv"
    )
...
def df_verao(html):
    ...
    calendario_verao_colheita_unica.to_csv(
        "docs_bronze/calendario_verao_colheita_unica.csv"
    )
    calendario_verao_colheita_multipla = lista[4][0]
    calendario_verao_colheita_multipla.to_csv(
        "docs_bronze/calendario_verao_colheita_multipla.csv"
    )
...
def df_outono(html):
    ...
    calendario_outono_aniversario.to_csv(
        "docs_bronze/calendario_outono_aniversario.csv"
    )
    calendario_outono_colheita_unica = lista[3][0]
    calendario_outono_colheita_unica.to_csv(
        "docs_bronze/calendario_outono_colheita_unica.csv"
    )
    calendario_outono_colheita_multipla = lista[5][0]
    calendario_outono_colheita_multipla.to_csv(
        "docs_bronze/calendario_outono_colheita_multipla.csv"
    )
...
def df_inverno(html):
    ...
    calendario_inverno_aniversario.to_csv(
        "docs_bronze/calendario_inverno_aniversario.csv"
    )
...
def df_conjuntos(html):
    ...
    conjunto_plantacoes_primavera.to_csv(
        "docs_bronze/conjunto_plantacoes_primavera.csv"
    )
    ...
    conjunto_plantacoes_qualidade.to_csv(
        "docs_bronze/conjunto_plantacoes_qualidade.csv"
    )
    ...
    conjunto_peixes_especializados.to_csv(
        "docs_bronze/conjunto_peixes_especializados.csv"
    )
...
def df_fertilizantes(html):
    ...
    taxa_cultivo_fertilizante_basico.to_csv(
        "docs_bronze/taxa_cultivo_fertilizante_basico.csv"
    )
    taxa_cultivo_fertilizante_qualidade = lista[3][0]
    taxa_cultivo_fertilizante_qualidade.to_csv(
        "docs_bronze/taxa_cultivo_fertilizante_qualidade.csv"
    )
    taxa_cultivo_fertilizante_premium = lista[4][0]
    taxa_cultivo_fertilizante_premium.to_csv(
        "docs_bronze/taxa_cultivo_fertilizante_premium.csv"
    )
...
def df_culinaria(html):
    ...
    culinaria_mercadorias_artesanais.to_csv(
        "docs_bronze/culinaria_mercadorias_artesanais.csv"
    )
    ...
    culinaria_pratos_ingredientes.to_csv(
        "docs_bronze/culinaria_pratos_ingredientes.csv"
    )
...
def df_artesanato(html):
    ...
    artesanato_equipamentos_artesanais.to_csv(
        "docs_bronze/artesanato_equipamentos_artesanais.csv"
    )
    ...
    artesanato_equipamento_refino.to_csv(
        "docs_bronze/artesanato_equipamento_refino.csv"
    )
...
def df_mobilia(html):
    ...
    plantas_decorativas_chao1.to_csv(
        "docs_bronze/mobilia_plantas_decorativas_chao1.csv"
    )
    plantas_decorativas_chao2 = lista[27][0]
    plantas_decorativas_chao2.to_csv(
        "docs_bronze/mobilia_plantas_decorativas_chao2.csv"
    )
    plantas_decorativas_penduradas = lista[28][0]
    plantas_decorativas_penduradas.to_csv(
        "docs_bronze/mobilia_plantas_decorativas_penduradas.csv"
    )
    plantas_decorativas_sazonais = lista[29][0]
    plantas_decorativas_sazonais.to_csv(
        "docs_bronze/mobilia_plantas_decorativas_sazonais.csv"
    )
    pinturas = lista[30][0]
...
def df_construcoes_fazenda(html):
    ...
    estoque_carpintaria_permanente.to_csv(
        "docs_bronze/estoque_carpintaria_permanente.csv"
    )
    ...
    construcoes_fazenda_carpintaria_melhoria.to_csv(
        "docs_bronze/casa_construcoes_melhoria.csv"
    )
    construcoes_fazenda_carpintaria_renovacoes = lista[6][0]
    construcoes_fazenda_carpintaria_renovacoes.to_csv(
        "docs_bronze/casa_construcoes_renovacoes.csv"
    )


def processamento_geral():
